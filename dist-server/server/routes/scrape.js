...
function mergeTimeIntoDate(dateIso, releaseTime) {
    if (!releaseTime)
        return dateIso;
...
function fallbackExtract(text) {
    ...
    let type = "news";
    if (caption.includes("restock") || caption.includes("re-stock"))
        type = "restock";
    else if (caption.includes("collab") || caption.includes("collaboration"))
        type = "collab";
    else if (caption.includes("drop") || caption.includes("release") || caption.includes("launch"))
        type = "drop";
...
async function extractWithAI(text, brandName) {
...
function normalizeForCompare(v) { return v.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim(); }
function stripBrandSuffix(value, brandName) {
    if (!value)
        return value;
...
function isLikelyProductItem(item) {
    if (!/[a-z]/i.test(item))
        return false;
    if (/(pocket|stitch|hem|liner|lining|silhouette|button|seam|placket|gusset|thread|finish interior)/i.test(item))
        return false;
    return true;
}
function resolveReleaseDate(caption, aiReleaseDate, postDateIso, releaseTime = null) {
    const explicitDates = extractDatesFromText(caption);
    if (explicitDates.length > 0) {
        if (!postDateIso)
            return mergeTimeIntoDate(explicitDates[0], releaseTime);
    ...
    if (aiReleaseDate) {
        ...
        if (!Number.isNaN(parsed.getTime())) {
            ...
            if (!postDateIso)
                return hasTime ? parsed.toISOString() : mergeTimeIntoDate(parsed.toISOString(), releaseTime);
            const postDate = new Date(postDateIso);
            if (!Number.isNaN(postDate.getTime())) {
...
async function parseApifyPosts(results, brandName) {
    const posts = [];
    const items = results.filter((item) => item.caption || item.text);
    const batchSize = 5;
    for (let i = 0; i < items.length; i += batchSize) {
        const batch = items.slice(i, i + batchSize);
        const extractions = await Promise.all(batch.map((item) => {
        ...
        for (let j = 0; j < batch.length; j++) {
            ...
            const cleanItems = info.items.map((p) => stripBrandSuffix(p, brandName)).filter(Boolean);
...
router.post("/instagram", async (req, res) => {
    try {
        ...
        if (!apifyKey)
            return res.status(500).json({ success: false, error: "Apify API key not configured" });
        ...
        if (!instagramHandle)
            return res.status(400).json({ success: false, error: "Could not extract Instagram handle from URL" });
        const profileUrl = `https://www.instagram.com/${instagramHandle}/`;
        console.log("Scraping Instagram profile via Apify:", profileUrl);
        const actorRunResponse = await fetch(`https://api.apify.com/v2/acts/apify~instagram-profile-scraper/run-sync-get-dataset-items?token=${apifyKey}`, {
            method: "POST",
        ...
        if (!apifyResults || apifyResults.length === 0)
            return res.status(404).json({ success: false, error: "No data found for this profile" });
        ...
        const topLevelPosts = apifyResults.slice(1).filter((item) => item.shortCode || item.caption || item.text);
        ...
        if (bioReleaseDate || bioInfo.items.length > 0) {
            ...
            const bioItems = bioInfo.items.map((item) => stripBrandSuffix(item, brandName)).filter(Boolean);
...
router.post("/all-brands", async (_req, res) => {
    try {
        ...
        if (!monitoredBrands.length)
            return res.json({ success: true, message: "No brands to scrape" });
        const results = [];
