...
async function fixExperimentFiveMarchDate() {
    ...
    try {
        ...
        const updateRes = await client.query(`UPDATE brand_posts
       SET extracted_release_date = $1,
           extracted_product_name = 'Henley hoodie'
       WHERE id = $2
         AND extracted_release_date <> $1`, [CORRECT_RELEASE_DATE, EXPERIMENT_FIVE_POST_ID]);
        ...
        const siblingCheck = await client.query(`SELECT id FROM brand_posts
       WHERE brand_id = (SELECT brand_id FROM brand_posts WHERE id = $1)
         AND extracted_product_name = 'Daily denim V2'
         AND extracted_release_date = $2`, [EXPERIMENT_FIVE_POST_ID, CORRECT_RELEASE_DATE]);
        if (siblingCheck.rowCount === 0) {
            await client.query(`INSERT INTO brand_posts
         ...
         SELECT
           ...
           $1, raw_content
         FROM brand_posts
         WHERE id = $2`, [CORRECT_RELEASE_DATE, EXPERIMENT_FIVE_POST_ID]);
